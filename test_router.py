#!/usr/bin/env python3
"""
Test Router - Check API -> Model communication
"""
...
def test_ai_endpoint(port=8007):
    ...
    test_message = "Privet! Eto testovyi zapros dlya proverki svyazi."
    
    for model in models:
        print(f"\nTesting {model.upper()} model...")
        
        try:
            # Send POST request to /ai/chat
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"Status: {status_data.get('ai_service', 'unknown')}")
        else:
...
def find_server_port():
    ...
    ports_to_try = [8000, 8001, 8002, 8003, 8004, 8005, 8006, 8007, 8008, 8009, 8010]
